...
pub async fn check_and_update_dkim(
    ...
    println!("INFO email_auth_addr {:?}", email_auth_addr);
        ...
        .await?
    {
        ...
        println!("INFO public key registered");
        return Ok(());
    }
    ...
    println!("INFO selector {}", selector);
    ...
    println!("INFO DKIM oracle result {:?}", oracle_result);
    ...
    println!("INFO public_key_hash from oracle {:?}", public_key_hash);
    let signature = Bytes::from_hex(&oracle_result.signature[2..])?;
    info!(LOG, "signature {:?}", signature);
    println!("INFO signature {:?}", signature);
    ...
    println!("INFO DKIM registry updated {:?}", tx_hash);
    Ok(())
}
